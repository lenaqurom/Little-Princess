...
if(isset($_POST['flowerType'])) {
    ...
    $dbTable = $string[1];
    $imgId = $string[0];
    $qry= "select * from $dbTable WHERE ID = $imgId";
    $ress= $dbb->query($qry);
    $roww=$ress->fetch_assoc();
    $_SESSION['var'] = $dbTable;
}
...
//                $img_path = '../firstProject/uploads/'.$new_img_name;
                ...
                $query = "UPDATE $dbTabl SET URL ='$new_img_name' WHERE ID = $idd";
...
//            header("Location: index.php?error=$em");
    ...
    $qryy = "UPDATE $dbTabl SET name ='$fName', price = $pri WHERE ID = $idd";
    $result = mysqli_query($dbb, $qryy);
...
if(isset($_POST['del'])){
    $dbTab = $_SESSION['var'];
    $i=$_POST['del'];
    $sql = "DELETE FROM $dbTab WHERE  ID = $i";
    $dbb->query($sql);
...
<head>
    ...
    <title> Little Princess | Edit Categories</title>
...
<ul id="slide-out" class="side-nav fixed z-depth-2"   >
    <li class="center no-padding"  >
        <div  style="height: 180px; background-color: #0e0e0e ; color: #ffffff" >
            <div class="row" style="background-color: #0e0e0e ; color: #d9c6ae "  >
...
<header>
    ...
    <nav >
        <div class="nav-wrapper bg-secondary text-white "  style="background: #0e0e0e" >
            ...
            <a class="breadcrumb" style="color: #ffffff">Edit-pic</a>
...
<main>
    ...
    <div class="form-popup2" id="myForm2" style="padding: 30px">
        <a style="color: #8b8d78; font-size: 30px">Edit Product</a>
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" class="form-container2" method="post" enctype="multipart/form-data">


            <div class="col-25">
                <label for="ID"><b>Product ID</b></label>
                <input type="number" name="ID" value="<?php echo $roww['ID']?>" readonly>
                <label for="Name"><b>Product Name</b></label>
                <input type="text" name='Name' value="<?php echo $roww['name']?>">

                <br>
            </div>
            <div class="col-75">
                <label for="Price"><b>Price</b></label>
                <input type="number" name="Price" value="<?php echo $roww['price']?>">
                <label for="Picture"><b>Product Picture</b></label>
                <br>
                <!--                <img src="--><?php //echo $roww['URL']?><!--"  name="img" style="width: 150px; height: 150px">-->
                <br>
    ...
    <form class="modal-content" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
        <div class="container">
            ...
            <div class="clearfix">
                ...
                <button type="submit" class="deletebtn" id="closeForm2" name="del" value="<?php echo $roww['ID']?>">Delete</button>
            </div>
        </div>
    </form>
</div>
